...
const { Teacher, School, Psychiatrist, Admin, Ms , ASA , CIF , RC} = require("../models/user");
...
const sendOTP = async (req, res) => {
    ...
    const data = role === "admin" || role === "ms" ? await sendOTPemail(contactValue) : await sendOTPphone(contactValue);
      if (data && data.Status === "Success") {
        return res.status(200).json({ msg: "OTP sent successfully", sessionId: data.Details , number: contactInfo});
      } else {
